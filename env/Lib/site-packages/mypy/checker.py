...
class TypeChecker(NodeVisitor[None], CheckerPluginInterface):
    ...
    def _type_check_raise_python2(
        self, e: Expression, s: RaiseStmt, typ: ProperType
    ) -> None:
        ...
        if not s.legacy_mode and (
            ...
            if len(typ.items) >= 2 and isinstance(
                get_proper_type(typ.items[1]), NoneType
            ):
        ...
        if s.target:
            ...
            if not isinstance(target_type, NoneType):
                ...
                if not is_subtype(write_type, required_type):
                    self.fail(
                        message_registry.PYTHON2_PRINT_FILE_TYPE.format(
                            write_type,
                            required_type,
                        ),
                        s.target,
