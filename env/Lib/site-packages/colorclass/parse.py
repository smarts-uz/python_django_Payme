...
CODE_GROUPS = (
    tuple(
        set(
            str(i)
            for i in BASE_CODES.values()
            if i and (40 <= i <= 49 or 100 <= i <= 109)
        )
    ),  # bg colors
    tuple(
        set(
...
def prune_overridden(ansi_string):
    ...
    """
    multi_seqs = set(
        p for p in RE_ANSI.findall(ansi_string) if ";" in p[1]
