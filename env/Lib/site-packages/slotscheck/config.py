...
class PartialConfig:
    ...
    def load(cls, p: Path) -> "PartialConfig":
        ...
        if p.suffix == ".toml":
            ...
            raise ValueError(
                "Settings file with invalid extension. " "Expected .toml, .ini, or .cfg"
            )
    ...
    def _load_ini(cls, p: Path) -> "PartialConfig":
        ...
        return cls._load_confmap(
            {k: cfg.BOOLEAN_STATES.get(v, v) for k, v in cfg.items("slotscheck")}
...
def collect(from_cli: PartialConfig, cwd: Path, config: Optional[Path]) -> Config:
