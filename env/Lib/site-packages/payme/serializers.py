...
class MerchantTransactionsModelSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs) -> dict:
        ...
        """
        if attrs.get("user_id") is not None:  # Change order_id to user_id
            try:
                order = Order.objects.get(
                    id=attrs["user_id"]  # Change order_id to user_id
                )
                ...
                logger.error(
                    "Invalid amount for user: %s", attrs["user_id"]
                )  # Update log message
    ...
    def validate_user_id(self, user_id) -> int:  # Change order_id to user_id
        """
        ...
        """
        try:
            Order.objects.get(id=user_id)  # Change order_id to user_id
        except Order.DoesNotExist as error:
            logger.error(
                "Order does not exist user_id: %s", user_id
            )  # Update log message
