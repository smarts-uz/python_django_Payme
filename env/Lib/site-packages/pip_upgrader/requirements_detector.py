...
class RequirementsDetector(object):
    ...
    def _is_valid_requirements_file(filename):
        ...
        return (
            ...
            and mimetypes.guess_type(filename)[0] in ["text/plain", None]
        )
            ...
            for line in fh:
                if line.strip().startswith("-r "):
                    ...
                    included_filename = os.path.join(
                        os.path.dirname(filename), included_filename
                    )
                    if (
                        self._is_valid_requirements_file(included_filename)
                        and included_filename not in self.filenames
                    ):
