...
class SameTypeVisitor(TypeVisitor[bool]):
    ...
    def visit_unpack_type(self, left: UnpackType) -> bool:
        return isinstance(self.right, UnpackType) and is_same_type(
            left.type, self.right.type
        )
    ...
    def visit_union_type(self, left: UnionType) -> bool:
        if isinstance(self.right, UnionType):
            ...
            for left_item in left_rem:
                if not any(
                    is_same_type(left_item, right_item) for right_item in right_rem
                ):
                    return False
            ...
            for right_item in right_rem:
                if not any(
                    is_same_type(right_item, left_item) for left_item in left_rem
                ):
                    return False
