...
def _generate(server_secret, session_secret, form_id, time, salt):
    ...
    if type(form_id) is str:
        form_id = form_id.encode("utf-8")
    hasher = HASH_FUNCTION
    msg = hasher(
        b"|".join((salt, server_secret, session_secret, form_id, time.encode("ascii")))
    ).digest()
    return "|".join(
        (
            VERSION,
            ...
            binascii.hexlify(msg).decode("ascii"),
        )
    )
...
def valid(server_secret, session_secret, form_id, window, server_time, token):
    # pure function
    if len(token) != len(VERSION) + DATETIME_FORMAT_LENGTH + 3 + (
        2 * RANDOM_BITS // 8
    ) + (2 * HASH_FUNCTION_BITS // 8):
        return False
    if token.count("|") != 3:
        return False
    ...
    try:
        ...
        try:
            time = datetime.datetime.strptime(time, DATETIME_FORMAT).replace(
