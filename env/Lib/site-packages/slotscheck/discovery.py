    ...
    def filtername(
        ...
        return replace(
            ...
            content=frozenset(
                filter(
                    None,
                    (sub.filtername(__pred, prefix=new_prefix) for sub in self.content),
                )
            ),
        )
...
def module_tree(module: ModuleName, expected_location: Optional[AbsPath]) -> ModuleTree:
...
def _nested_classes(c: type) -> Iterable[type]:
    return unique(filter(partial(_is_nested_class, parent=c), c.__dict__.values()))
