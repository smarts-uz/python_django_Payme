...
class SemanticAnalyzer(
    ...
    def process_imported_symbol(
        ...
        module_hidden = not module_public and not (
            ...
            # re-exports module
            isinstance(node.node, MypyFile)
            and fullname in self.modules
        )
    ...
    def extract_typevarlike_name(
        self, s: AssignmentStmt, call: CallExpr
    ) -> Optional[str]:
        if not call:
            return None
...
def refers_to_fullname(
    node: Expression, fullnames: Union[str, Tuple[str, ...]]
) -> bool:
