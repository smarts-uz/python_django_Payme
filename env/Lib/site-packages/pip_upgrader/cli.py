...
def main():
    ...
    try:
        ...
        packages_status_map = PackagesStatusDetector(
            packages, options.get("--use-default-index")
        ).detect_available_upgrades(options)

        # 4. [optionally], show interactive screen when user can choose which packages to upgrade
        selected_packages = PackageInteractiveSelector(
            packages_status_map, options
        ).get_packages()

        # 5. having the list of packages, do the actual upgrade and replace the version inside all filenames
        upgraded_packages = PackagesUpgrader(
            selected_packages, filenames, options
        ).do_upgrade()
        ...
        print(
            Color(
                ...
                "{}{{/autogreen}}".format(
                    ",".join([package["name"] for package in upgraded_packages])
                )
            )
        )
        if options["--dry-run"]:
