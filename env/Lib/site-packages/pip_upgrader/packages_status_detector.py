...
class PackagesStatusDetector(object):
    ...
    def _update_index_url_from_configs(self):
        ...
        if "VIRTUAL_ENV" in os.environ:
            self.pip_config_locations.append(
                os.path.join(os.environ["VIRTUAL_ENV"], "pip.conf")
            )
            self.pip_config_locations.append(
                os.path.join(os.environ["VIRTUAL_ENV"], "pip.ini")
            )
        ...
        if index_url:
            ...
            print(
                Color(
                    ...
                    ". Use --default-index-url to use pypi default index".format(
    ...
    def detect_available_upgrades(self, options):
        ...
        for i, package in enumerate(self.packages):
            try:
                ...
                if (
                    explicit_packages_lower
                    and package_name.lower() not in explicit_packages_lower
                ):
                ...
                if pinned_version and isinstance(
                    current_version, version.Version
                ):  # version parsing is correct
                    ...
                    print(
                        "{}/{}: {} ... ".format(
                            i + 1, len(self.packages), package_name
                        ),
                        end="",
                    )
                    sys.stdout.flush()
    ...
    def _fetch_index_package_info(self, package_name, current_version):
        ...
        try:
            ...
            response = requests.get(
                self.PYPI_API_URL.format(package=package_canonical_name), timeout=15
            )
        ...
        if self.PYPI_API_TYPE == "pypi_json":
            return self._parse_pypi_json_package_info(
                package_name, current_version, response
            )
    ...
    def _parse_pypi_json_package_info(self, package_name, current_version, response):
        ...
        if (
            self._prerelease
            or current_version.is_postrelease
            or current_version.is_prerelease
        ):
            prerelease_versions = [
                vers
                for vers in all_versions
                if vers.is_prerelease or vers.is_postrelease
            ]
    ...
    def _parse_simple_html_package_info(self, package_name, current_version, response):
        ...
        :type response: requests.models.Response
        """
        pattern = r"<a.*>.*{name}-([A-z0-9\.-]*)(?:-py|\.tar).*<\/a>".format(
            name=re.escape(package_name)
        )
        versions_match = re.findall(
            pattern, response.content.decode("utf-8"), flags=re.IGNORECASE
        )
