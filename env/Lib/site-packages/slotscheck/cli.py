...
@click.option(
    ...
    help="Report an error when a slots class inherits from " "a non-slotted class.",
    default=None,
...
@click.option("-v", "--verbose", is_flag=True, help="Display extra descriptive output.")
@click.option(
    "--settings",
...
class OverlappingSlots(Notice):
    ...
    def for_display(self, verbose: bool) -> str:
        return f"'{_class_fullname(self.cls)}' defines overlapping slots." + verbose * (
...
def _duplicate_slots(c: type) -> Iterable[str]:
    return (slot for slot, count in Counter(slots(c) or ()).items() if count > 1)
...
def _as_modules(
    ...
    if files and names:
        print(
            _format_error(
                "Specify either FILES argument or `-m/--module` " "option, not both."
            ),
            file=sys.stderr,
        )
        exit(2)
...
def _class_excludes(classes: Iterable[type], exclude: Optional[str]) -> Iterable[type]:
    return (
        filter(
            compose(
                not_,
...
def _class_includes(classes: Iterable[type], include: Optional[str]) -> Iterable[type]:
    return (
        filter(
            compose(
...
def _format_note(msg: str) -> str:
    return (
        _NOTE_PREFIX + indent(msg, " " * len(_NOTE_PREFIX))[len(_NOTE_PREFIX) :]  # noqa
    )
